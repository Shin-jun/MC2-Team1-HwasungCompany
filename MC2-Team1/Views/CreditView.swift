        ...
        ZStack {
            ...
            Text(attributedString)
                ...
                .onAppear() {
                    ...
                    attributedString[range1].font = .custom(modelData.contentFontName, size: 20)
                    let range2 = attributedString.range(of: "영양사")!
                    attributedString[range2].font = .custom(modelData.contentFontName, size: 20)
                    let range3 = attributedString.range(of: "충전")!
                    attributedString[range3].font = .custom(modelData.contentFontName, size: 20)
                    let range4 = attributedString.range(of: "글루건")!
                    attributedString[range4].font = .custom(modelData.contentFontName, size: 20)
                    let range5 = attributedString.range(of: "원어민")!
                    attributedString[range5].font = .custom(modelData.contentFontName, size: 20)
                    let range6 = attributedString.range(of: "과장")!
                    attributedString[range6].font = .custom(modelData.contentFontName, size: 20)
